...
class _GraphState extends State<Graph> {
  ...
  Widget build(BuildContext context) {
    ...
    const double leftMax = leftUpperLimit + (leftUpperLimit - leftLowerLimit) / 2;
    const double leftMin = leftLowerLimit - (leftUpperLimit - leftLowerLimit) / 2;
    ...
    for (int i = 0; i < data.length; i++) {
      rightSpots.add(FlSpot(data[i].unixTime.toDouble(), data[i].right));
    }

    // Create the left FlSpot points
    List<FlSpot> leftSpots = [];
    for (int i = 0; i < data.length; i++) {
      leftSpots.add(FlSpot(data[i].unixTime.toDouble(), convertLeftValue(data[i].left)));
    }
